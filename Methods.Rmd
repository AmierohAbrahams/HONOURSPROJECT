...
=======
  ...
  ungroup() %>% 
  slice(1:4) 
>>>>>>> 51fd8d58146e317b0c285bbeb979ab2a51550192
...
>>>>>>> 51fd8d58146e317b0c285bbeb979ab2a51550192
  group_by(src) %>%
<<<<<<< HEAD
  filter(length %in% head(length, 1)) %>%
  ungroup()
||||||| merged common ancestors
  filter(length %in% head(length, 1)) %>% 
  ungroup()
...
clust_plot <- function(df) {
  plot1 <- ggplot(df, aes(x = index_pair)) +
    geom_boxplot(aes(y = temp_1), fill = "khaki4", alpha = 0.3, position_dodge(width = 0.2)) +
...
```{r run_fun3}
SACTN_clust_1_match %>% 
  gather(key = "index", value = "temp", temp_1, temp_2, -date, -index_pair)
...
```{r}
clust_plot_individual <- function(df) {
  plot1_individual <- ggplot(df, aes(x = index_pair)) +
    #geom_boxplot(aes(y = temp_1), fill = "khaki4", alpha = 0.3) +
    geom_boxplot(aes(y = temp_2), fill = "chartreuse3", alpha = 0.3) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  return(plot1_individual)
}
```

```{r}
SACTN_clust_1_plot1_individual <- clust_plot_individual(df = SACTN_clust_1_match)
SACTN_clust_1_plot1_individual
SACTN_clust_2_plot1_individual <- clust_plot_individual(df = SACTN_clust_2_match)
SACTN_clust_2_plot1_individual
SACTN_clust_3_plot1_individual <- clust_plot_individual(df = SACTN_clust_3_match)
SACTN_clust_3_plot1_individual
SACTN_clust_4_plot1_individual <- clust_plot_individual(df = SACTN_clust_4_match)
SACTN_clust_4_plot1_individual
```

```{r}
clust1_plot_individual <- function(df) {
  plot1_individual <- ggplot(df, aes(x = index_pair)) +
    geom_boxplot(aes(y = temp_1), fill = "khaki4", alpha = 0.3) +
    #geom_boxplot(aes(y = temp_2), fill = "chartreuse3", alpha = 0.3) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
SACTN_clust_1_plot1.1_individual <- clust1_plot_individual(df = SACTN_clust_1_match)
SACTN_clust_1_plot1.1_individual
SACTN_clust_2_plot1.1_individual <- clust1_plot_individual(df = SACTN_clust_2_match)
SACTN_clust_2_plot1.1_individual
SACTN_clust_3_plot1.1_individual <- clust1_plot_individual(df = SACTN_clust_3_match)
SACTN_clust_3_plot1.1_individual
SACTN_clust_4_plot1.1_individual <- clust1_plot_individual(df = SACTN_clust_4_match)
SACTN_clust_4_plot1.1_individual
```

```{r}
cluster_1 <- ggarrange(SACTN_clust_1_plot1_individual,SACTN_clust_1_plot1.1_individual)
cluster_1
cluster_2 <- ggarrange(SACTN_clust_2_plot1_individual,SACTN_clust_2_plot1.1_individual)
cluster_2
cluster_3 <- ggarrange(SACTN_clust_3_plot1_individual,SACTN_clust_3_plot1.1_individual)
cluster_3
cluster_4 <- ggarrange(SACTN_clust_4_plot1_individual,SACTN_clust_4_plot1.1_individual)
cluster_4
```
...
On a monthly basis, between 1990 to 2016, the rates of changes in differences of average temperatures between sites within cluster 1 (Cape Agulhas, Gansbaai, Mossel Bay, and Ystervarkpunt) varied on an apparent seasonal basis. During the summer months we saw that there were large differences in average temperatures between Gansbaai and the remaining three sites, and that the rate of changes between these rapidly increased on yearly basis. Towards the end of summer and autumn months, in addition to the continuing trends observed between Gansbaai and the remaining sites we then also saw an increase in the rate of change in differences between Cape Agulhas and Mossel Bay, and Ystervarkpunt respectively. During winter we see very little differences in average temperatures between all four sites, and this remained relatively stable over the time period. During spring months there was an increase in the rate of change of average temperatures between Cape Agulhas and Gansbaai which continues into summer. 
Conversely to the first cluster, in the cluster containing Kalk Bay, Gordons Bay, Muizenberg and Tsitsikamma the largest rates of changes were observed during autumn and winter months. In this cluster we saw large differences in average temperatures between Muizenberg and the remaining sites, with the differences increasing annually throughout 1972 and 2016 during winter. Similarly, the rates of changes in differences of average temperatures also increased between Kalk Bay and Muizenberg during these same months. In the summer and spring months we saw relatively little differences in average temperatures between sites, with minimal differences in the rates of these changes over the 44 year time period. These rates increased during spring.
...
<<<<<<< HEAD
After the sites were matched the distance was converted to radians the sites were matched with the sites of the 10 year time series. Hereafter the wave data were combined with the temperature data at a depth of 7 and 15m respectivley.
||||||| merged common ancestors
After the sites were matched the distance was converted to radians the sites were matched with the sites of the 10 year time series. Hereafter the wave data were combined with the temperature data at a depth of 7 and 15m respectivley. 
=======
